...
usage() {
  ...
  safe_echo "  --ip            Set the IP address (can also accept a valid interface name)"
  safe_echo "  --port          Set the listening port"
  safe_echo "  --message       Set the default message to send"
  safe_echo "  --topic         Set the default topic to send"
